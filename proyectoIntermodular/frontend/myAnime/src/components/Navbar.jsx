...
function Navbar() {
  ...
  const enSesiones = location.pathname.startsWith("/login") || location.pathname.startsWith("/register");
  ...
  return (
    <MDBNavbar
      ...
      light={!darkMode}
      dark={darkMode}
      className={darkMode ? "navbar-dark-custom py-2" : "navbar-light-custom py-2"}
    >
      <MDBContainer fluid className="flex-column">
        ...
        <div className="d-flex align-items-center w-100">
          ...
          {/* LOGO */}
          <MDBNavbarBrand tag={Link} to="/" className="d-flex align-items-center gap-2">
            <MDBIcon fas icon="tv" />
            AniManga
          </MDBNavbarBrand>
          ...
          {isLoggedIn ? (
            ...
            <div className="d-flex align-items-center gap-2">
              <Link to="/login">
                <MDBBtn color="link" style={{ color: darkMode ? "#fff" : "#000", padding: "0.5rem 0.75rem" }}>
                  Iniciar Sesión
                </MDBBtn>
              </Link>
              <Link to="/register">
  ...
  {activeTab && !enDetalleObra && !enSesiones && (
    <div className={`nav-slider ${activeTab === "anime" ? "active-anime" : "active-manga"}`} />
  )}

  {enDetalleObra || enSesiones ? (
    <>
      <MDBBtn color="link" onClick={() => navigate(-1)} className="nav-tab">
